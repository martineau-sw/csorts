...
void merge_sort_strings(size_t start, size_t end, char array[end-start][255]) {
    if (start == end) {
        return;
    }

    size_t mid = (start+end)/2;
    merge_sort_strings(start, mid, array);
    merge_sort_strings(mid+1, end, array);
    
    size_t left_length = (mid-start)+1;
    size_t right_length = (end-mid);

    char left[left_length][255];
    char right[right_length][255];
    
    for (size_t i = 0; i < left_length; i++) {
        strcpy(left[i], array[start+i]);
    }
    for (size_t i = 0; i < right_length; i++) {
        strcpy(right[i], array[mid+1+i]);
    }
